...
# for the last trading day, appends to master, and regenerates derived
# datasets (etf_ohlcv, etf_returns, etf_vol, macro_fred).
# (macro_derived is not updated here; it will be regenerated by training.)
...
def fetch_fred_row(target_date: pd.Timestamp) -> pd.Series:
    """Fetch FRED data for a single date using direct API (no fredapi)."""
...
def update_master() -> None:
    ...
    ret_df = load_parquet(cfg.FILE_ETF_RETURNS)
    if not ret_df.empty and len(ret_df) >= 21:
        ...
        existing_vol = load_parquet(cfg.FILE_ETF_VOL)
    ...
    # ------------------------------------------------------------------
    # Note: macro_derived.parquet is not updated here.
    # It can be regenerated by the training pipeline when needed.

    log.info("All datasets updated successfully (except macro_derived).")
